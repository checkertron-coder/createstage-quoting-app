...
class BaseCalculator(ABC):
    ...
    def _build_from_ai_cuts(self, job_type: str, ai_cuts: list,
        ...
        if trust_opus:
            # Calculate from actual cut dimensions (math, not an override)
            total_area_sqin = 0.0
            for cut in ai_cuts:
                l = cut.get("length_inches", 0)
                w = cut.get("width_inches", 0)
                q = cut.get("quantity", 1)
                if w > 0:
                    total_area_sqin += l * w * q  # sheet/plate: L × W
                else:
                    total_area_sqin += l * 4 * q  # tube/bar: L × ~4" perimeter avg
            total_sq_ft = round(total_area_sqin / 144.0, 1)
        else:
        ...
        # Full package path: Opus includes laser in hardware if needed — skip.
        # Legacy path: auto-detect and inject laser cutting hardware.
        if trust_opus:
            # Opus handles laser cutting — don't inject
            pass
        else:
            hardware = self._inject_laser_cutting(ai_cuts, fields, hardware, assumptions)
    ...
    def _inject_laser_cutting(self, ai_cuts, fields, hardware, assumptions):
        """Legacy path: auto-detect laser cutting needs and inject hardware item.
        Returns the (possibly new) hardware list."""
    ...
    def _build_from_full_package(self, job_type: str, package: dict,
        ...
        # Convert Opus hardware to HardwareItem format — single pricing option
        # (trust Opus's price, no fake "Premium" markup tier)
        opus_hardware = []
        ...
        # Build material list — trust_opus=True skips overrides
        # (no waste factor, no laser injection, no hardcoded weld/area)
