...
def test_base_build_from_ai_cuts_produces_valid_material_list():
    ...
    # Items are consolidated by profile (what you buy from supplier)
    assert len(result["items"]) == 1
    assert "pipe_6_sch40" in result["items"][0]["description"]
    assert result["items"][0]["profile"] == "pipe_6_sch40"
    # Cut list preserves per-piece detail
    assert "cut_list" in result
    assert len(result["cut_list"]) == 1
    assert result["cut_list"][0]["description"] == "Bollard pipe"
    assert result["cut_list"][0]["quantity"] == 3
    assert "Test assumption" in result["assumptions"]
    assert any("Cut list generated by AI" in a for a in result["assumptions"])
